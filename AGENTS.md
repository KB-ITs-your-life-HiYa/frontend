...
├── services/      api.ts (공통 클라이언트), auth.ts (토큰 저장소), 도메인별 API
├── types/         공통 타입
├── hooks/         커스텀 훅
└── utils/         confirm.ts 등
```
...
const notice = await api.get<HousingNoticeDetail>('/housing/notices/12')
```
...
| `EXPO_PUBLIC_SUPABASE_URL` | 미사용 Supabase 자리 표시자용 변수 |
| `EXPO_PUBLIC_SUPABASE_ANON_KEY` | 미사용 Supabase 자리 표시자용 변수 |
...
- **커밋 메시지에 `Co-authored-by:` 트레일러를 절대 넣지 않는다.** 자동으로 추가되는 공동 작성자 표기도 제거한다.
- 커밋 메시지, PR 제목·본문, 코드 주석, 문서에 AI 도구의 자동 서명이나 기여 표기(예: `Generated by`, `Authored by`, `🤖 Generated with`)를 남기지 않는다.
- 커밋·PR 작성 전에 위 표기가 포함되지 않았는지 확인한다. 프로젝트 규칙이나 기술 설명에 필요한 도구명 언급은 기여 표기에 해당하지 않는다.
